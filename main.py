...
def classify_material(cls_name, frame, box):
    ...
    if cls_name == "bottle":
        ...
        var = np.var(gray)
        return "metal bottle" if var > 2500 else "plastic bottle"
    if cls_name == "cup":
        x1, y1, x2, y2 = map(int, box)
        crop = frame[y1:y2, x1:x2]
        brightness = np.mean(crop)
        return "paper cup" if brightness > 140 else "plastic cup"
    return cls_name
...
def gen_frames():
    ...
    while True:
        ...
        for box, cls, conf in zip(result.boxes.xyxy, result.boxes.cls, result.boxes.conf):
            ...
            cv2.putText(annotated_frame, f"{material_label} {conf:.2f}",
                        (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX,
                        0.6, (0, 255, 0), 2)
        timestamp = datetime.now(timezone.utc).isoformat()
        annotated_frame = add_visible_watermark_cv2(annotated_frame, f"Secure Detection | {timestamp}")
        hash_val = compute_sha256(annotated_frame)
        cv2.putText(annotated_frame, f"SHA256: {hash_val[:12]}...",
...
def index():
    ...
    <html>
    <head>
        <title>Material Detection</title>
        <style>
    ...
    <body>
        <h1>Material-Based Waste Detection</h1>
